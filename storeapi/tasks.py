    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                data={
                    "from": f"Lawrence Chuang <mailgun@{config.MAILGUN_DOMAIN}>",
                    "to": [to],
                    "subject": subject,
                    "text": body,
                },
...
async def send_user_registration_email(email: str, confirmation_url: str):
    ...
    return await send_simple_email(
        ...
        (
            f"Hi {email}! You have successfully signed up for our service.\n"
...
async def generate_and_add_to_post(
    ...
    post_url: str,
    database: Database,
    prompt: str = "A blur british shorthair cas is sitting on the courch.",
):
    try:
    ...
    await send_simple_email(
        ...
        (
            f"Hi {email}! Your image has been generated and added to you post."
